...
from taskflow.stark_parser import StarkSegment, parser_objectif_stark
...
def _construire_message_micro_objectif(
    ...
    return (
        ...
        "3. DÉCISION :\n"
        f"   - Si l'objectif \"{etat.objectif}\" est déjà atteint par l'état actuel : appelle terminer_tache directement avec un résumé clair.\n"
...
def executer_interaction_utilisateur(
    ...
    if origine_vocale:
        ...
        def _tts_worker(texte_a_lire: str) -> None:
            from jarvis.voice_output import parler_a_voix_haute
...
class AutonomousAgent:
    ...
    def agir(self, actions: list[dict]) -> list[str]:
        from datashield.safety import action_bloquee
