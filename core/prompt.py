...
def construire_prompt_conversation(memoire: dict) -> str:
    ...
    return (
        ...
        f"Ton inventaire de capacités vient du registre d'outils actif. Si l'utilisateur demande ce que tu sais faire, consulte tes capacités actuelles plutôt que de réciter une liste mémorisée.\n"
...
def construire_prompt_action(memoire: dict, taches_en_cours: list = None) -> str:
    ...
    return (
        ...
        f"Ton inventaire de capacités est généré en temps réel depuis le registre d'outils actif. "
        f"Si l'utilisateur te demande ce que tu peux faire, appelle lire_capacites puis résume honnêtement les capacités disponibles.\n\n"
        f"Contexte systeme :\n"
        f"- OS : {os_detecte}\n"
        f"- Dossier home : {home}\n"
        f"- Langue : {langue}\n"
        ...
        + "- Les lectures fichier/dossier sont libres. Les ecritures sont libres dans l'espace utilisateur.\n"
        + "- Les ecritures vers JARVIS_DIR ou les zones systeme detectees demandent confirmation interactive, sauf en mode Stark.\n"
